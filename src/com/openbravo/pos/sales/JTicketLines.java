...
public class JTicketLines extends javax.swing.JPanel {
    private static TicketProductNoteDAO td= new TicketProductNoteDAO();
    protected static DataLogicSales dlSales = null;
    ...
    public JTicketLines(String ticketline, DataLogicSales dlReceipts) { //nambah parameter tuk dapat property
        System.out.println("Tiketlines "+ticketline);
        this.dlSales = dlReceipts;
        initComponents();
    ...
    public void addTicketLine(TicketLineInfo oLine) {
        ...
        //System.out.println("@@@@@ propiedades de linea: "+ oLine.getProperties().toString() );
        // Selecciono la que acabamos de anadir.            
        setSelectedIndex(m_jTableModel.getRowCount() - 1);
    }

    public void addTicketLine(TicketLineInfo oLine, Object ticketInfo) {

        m_jTableModel.addRow(oLine, ticketInfo);
        //System.out.println("@@@@@ propiedades de linea: "+ oLine.getProperties().toString() );
        // Selecciono la que acabamos de anadir.            
        setSelectedIndex(m_jTableModel.getRowCount() - 1);
    }
...
//        private AppView m_App;
        ...
        public void setRow(int index, TicketLineInfo oLine) {
            ...
            Enumeration e = oLine.getProperties().propertyNames();

            for (; e.hasMoreElements();) {
                System.out.println("set" + e.nextElement());
        ...
        public void addRow(TicketLineInfo oLine, Object ticketInfo) {

            insertRow(m_rows.size(), oLine, ticketInfo);
        }
        ...
        public void insertRow(int index, TicketLineInfo oLine) {
            if (dlSales != null) {
                try {
                    ProductInfoExt p = dlSales.getProductInfo(oLine.getProductID());
                    Enumeration e = p.getProperties().propertyNames();

                    for (; e.hasMoreElements();) {
                        String key = e.nextElement().toString();
                        if (oLine.getProperty(key) == null) {
                            oLine.setProperty(key, p.getProperty(key));
                        }
                        //  System.out.println("isi "+key+" : "+p.getProperty(key));

                    }

                    // TicketProductNote t = dlSales.getTicketProductNote(oLine., TOOL_TIP_TEXT_KEY)

                } catch (BasicException ex) {
                    ex.printStackTrace();
                    Logger.getLogger(JTicketLines.class.getName()).log(Level.SEVERE, null, ex);
                }
            ...
            for (; e.hasMoreElements();) {
                System.out.println("insert " + e.nextElement());
        ...
        public void insertRow(int index, TicketLineInfo oLine, Object ticketInfo) {
            if (dlSales != null) {
                try {
                    //System.out.println("########## oLine.getProductID : " + oLine.getProductID());
                    ProductInfoExt p = dlSales.getProductInfo(oLine.getProductID());
                    Enumeration e = p.getProperties().propertyNames();
                    System.out.println("########## properties : "+p.getProperties().propertyNames());
                    ...
                    //TicketProductNote t = dlSales.getTicketProductNote(ticketInfo.getId(), oLine.getProductID());
                    String ticket=ticketInfo.toString();
                    System.out.println("Ticket hhh "+ticket);
                    if(ticket.startsWith("0")){
                        ticket=ticket.substring(1, ticket.length());
                        System.out.println("Ticket hhhqqq "+ticket);
                    }
                    TicketProductNote t = td.findByTicket(ticket, oLine.getProductID());
                    if (t != null) {
                        oLine.setProperty("notes", t.getNote());
                    }
