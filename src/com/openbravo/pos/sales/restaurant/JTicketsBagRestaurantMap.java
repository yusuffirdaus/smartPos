...
import com.openbravo.pos.sales.PrinterSelector;
...
//    private static final Icon ICO_FRE = new NullIcon(22, 22);
    ...
    public void setDuration(){
        ...
        com.openbravo.pos.sales.restaurant.Time t = new com.openbravo.pos.sales.restaurant.Time();
        com.openbravo.pos.sales.restaurant.TimeEntity te = t.timeFormat(s, m, h);
            ...
            try {
                if(isTicketCancel){
                    ...
                    for(TicketLineInfo line: ticket.getLines()){ //--
                        if ((line.getProperty("sendstatus").equals("Yes")||line.getProperty("sendstatus").equals(null)) && line.getProductID() != null) 
...
/**
     * Request a invetory shipping to the ERP  -      SmartPOS
     * @param ticket
     * @param line
     * @param reason 
     */
    ...
    private static class printerMapping {
            ...
            if (m_PlaceClipboard == null) {  
                if (customer == null) {
                    ...
                    if (ticket == null && !m_place.hasPeople()) {
                        ...
                        /*
                        ticket.setUser(m_App.getM_principalapp().getUser().getUserInfo());
                        ticket.setUser(m_App.getAppUserView().getUser().getUserInfo());
                        ticket.setProperty("user", m_App.getM_principalapp().getUser().getName());
                        */
                        try {
                     ...
                     /*{ @win
                        ...
                        // here the it access the ticket just if the POS station is the same (user)
                        logger.severe("usuario del ticket:"+ticket.getProperty("user"));
                        String currentUser =  m_App.getM_principalapp().getUser().getName();
                        logger.severe("usuario autenticado:"+currentUser);
                        String rol = m_App.getM_principalapp().getUser().getRole();
                        Boolean rolAdmin = (rol.equalsIgnoreCase("1" ) || rol.equalsIgnoreCase("2" )) ;
                        
                       // just enable to modify ticket os the same owner or if it is a administrator (roles 1 or 2)
                        if ((ticket.getProperty("user").equalsIgnoreCase(currentUser)) || rolAdmin ) 
                        */
                        {
                            setActivePlace(m_place, ticket);                   
                       }
                        /*else {
                            new MessageInf(MessageInf.SGN_WARNING,  AppLocal.getIntString("message.ticketblocked")+ticket.getProperty("user") ).show(JTicketsBagRestaurantMap.this);       
                       }          
                    }*/
                } else {
                    // receiving customer.
                    ...
                    if (ticket == null) {
                        ...
                        //ticket.setUser(m_App.getM_principalapp().getUser().getUserInfo());
                        //ticket.setUser(m_App.getAppUserView().getUser().getUserInfo());
                        logger.severe("Usuario asignado al ticket:"+m_App.getM_principalapp().getUser().getName());
                        ticket.setProperty("user", m_App.getM_principalapp().getUser().getName());
                        try {
