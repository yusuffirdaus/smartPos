...
import com.crm.thread.UpdateSoldThread;
...
import com.openbravo.pos.dao.CustomerDAO;
...
public class DataLogicSales extends BeanFactoryDataSingle {
    ...
    public final ProductInfoExt getProductInfo(String id) throws BasicException {
        System.out.println("############## ProductInfo : "+ id);
    ...
    public final SentenceList getProductList() {
        return new StaticSentence(s
            , new QBFBuilder(
              "SELECT ID, REFERENCE, CODE, NAME, ISCOM, ISSCALE, PRICEBUY, PRICESELL, TAXCAT, CATEGORY, ATTRIBUTESET_ID, IMAGE, ATTRIBUTES, PRICESTD, PRICELIMIT, UNIT " +
              "FROM PRODUCTS WHERE ?(QBF_FILTER) ORDER BY NAME", new String[] {"NAME", "PRICEBUY", "PRICESELL", "CATEGORY", "CODE"})
    ...
    public final SentenceList getProductListExt() {
        return new StaticSentence(s
            , new QBFBuilder(
              "SELECT ID, REFERENCE, CODE, NAME, ISCOM, ISSCALE, PRICEBUY, PRICESELL, TAXCAT, CATEGORY, ATTRIBUTESET_ID, IMAGE, ATTRIBUTES, PRICESTD, PRICELIMIT, UNIT " +
              "FROM PRODUCTS WHERE ?(QBF_FILTER) ORDER BY NAME", new String[] {"NAME", "PRICEBUY", "PRICESELL", "CATEGORY", "CODE"})
    ...
    /**
     * Busca clientes por TAX ID - SmartPOS
     */
    public CustomerInfoExt loadCustomerExtByTaxId(String id) throws BasicException {
        return (CustomerInfoExt) new PreparedSentence(s
                , "SELECT ID, TAXID, SEARCHKEY, NAME, CARD, TAXCATEGORY, NOTES, MAXDEBT, VISIBLE, CURDATE, CURDEBT" +
                  ", FIRSTNAME, LASTNAME, EMAIL, PHONE, PHONE2, FAX" +
                  ", ADDRESS, ADDRESS2, POSTAL, CITY, REGION, COUNTRY, tax_exempt" +
    ...
    public final synchronized void saveTicket(final TicketInfo ticket, final String location) throws BasicException {
        ...
        Transaction t = new Transaction(s) {
            public Object transact() throws BasicException {
                ...
                // new ticket
                new PreparedSentence(s
                    , "INSERT INTO TICKETS (ID, TICKETTYPE, TICKETID, PERSON, CUSTOMER, WAITER, PLACE, FLOOR, MALE, FEMALE) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
                    , SerializerWriteParams.INSTANCE
                    ).exec(new DataParams() { public void writeValues() throws BasicException {
                        setString(1, ticket.getId());
                        ...
                        setString(7, ticket.getPlace());
                        setString(8, ticket.getFloor());
                        setInt(9, ticket.getMale());
                        setInt(10, ticket.getFemale());
                    }});

                SentenceExec ticketlineinsert = new PreparedSentence(s
                    , "INSERT INTO TICKETLINES (TICKET, LINE, PRODUCT, ATTRIBUTESETINSTANCE_ID, UNITS, PRICE, TAXID, ATTRIBUTES,UNITSID, NOTE) VALUES (?, ?, ?, ?, ?, ?, ?, ?,?,?)"// add column note @ade
                    , SerializerWriteBuilder.INSTANCE);
                int i=0;                
                for (TicketLineInfo l : ticket.getLines()) {
                    Main.logger.info(" Save tiketId : "+ticket.getId()+" line : "+ l.getTicketLine());
                    //ProductDAO.getInstance().updateSoldProduct(l.getProductID(), l.printMultiply());
                    l.setTicketLine(i);                    
                 ...
                 List<TicketLineInfo> tls =new ArrayList<TicketLineInfo>();
                
                 for (TicketLineInfo l : ticket.getLines()) {
                     
                     boolean ada=false;
                     for (TicketLineInfo ticketLineInfo : tls) {
                         if(ticketLineInfo.getProductID().equalsIgnoreCase(l.getProductID())){
                             ada=true;
                             ticketLineInfo.setMultiply(ticketLineInfo.getMultiply()+l.getMultiply());
                             break;
                         }
                     }
                     
                     if(!ada){
                         TicketLineInfo  tt= new TicketLineInfo();
                         tt.setProductID(l.getProductID());
                         tt.setMultiply(l.getMultiply());
                         tls.add(tt);
                     }
                 }
                 
                 new Thread(new UpdateSoldThread(tls)).start();
                 
                System.out.println("Save Customer");
                //Save Customer @ade
                try{
//                if(ticket.getProperties()!=null){
//                if(ticket.getProperty("customer")!=null){
//                    //Customer c= Util.gson2.fromJson(ticket.getProperty("customer"), Customer.class);
                    Date date = ticket.getDate();
                    String id = ticket.getId();
                    String place= ticket.getPlace();
                    //String floor=ticket.getFloor();
                    int male = ticket.getMale();
                    int female = ticket.getFemale();
                    
                    CustomerDAO.getInstance().insert(date,id,place,male,female);       
                    //Main.logger.info("save customer"+ticket.getProperty("customer"));
                    //}
                //}
                System.out.println("Save Customer Done "+ ticket.getId());
                }catch(Exception e){
                    Main.logger.info("Save Customer Error : ",e );
                }
...
//                    , SerializerWriteString.INSTANCE).exec(ticket.getId());
    ...
    public final double findProductStockByLocation(String warehouse, String id, String attsetinstid) throws BasicException {
        ...
        public final double findProductStock(String id, String attsetinstid) throws BasicException {

        PreparedSentence p = attsetinstid == null
                ? new PreparedSentence(s, "SELECT UNITS FROM STOCKCURRENT WHERE PRODUCT = ? AND ATTRIBUTESETINSTANCE_ID IS NULL"
                    , new SerializerWriteBasic(Datas.STRING)
                    , SerializerReadDouble.INSTANCE)
                : new PreparedSentence(s, "SELECT UNITS FROM STOCKCURRENT WHERE PRODUCT = ? AND ATTRIBUTESETINSTANCE_ID = ?"
                    , new SerializerWriteBasic(Datas.STRING, Datas.STRING)
                    , SerializerReadDouble.INSTANCE);

        Double d = (Double) p.find( id, attsetinstid);
        return d == null ? 0.0 : d.doubleValue();
    }
    ...
    /**
     * Retorna un registro con los factores de conversion (multiply & divide para la unidad u produco especificados)
     * @param productID
     * @param unitId
     * @return 
     */
    public final double getConversionFactorsUOMProduct(String productID,String unitId, String unitToId)   {
        Double d = null;
        try {
            PreparedSentence p = new PreparedSentence(s
                    , "SELECT divide_rate "+
                      "FROM additional_prices_for_products "+
                      "WHERE product_id = ?  AND unit_id = ? AND unit_to_id = ?  ORDER BY ID "
                    , new SerializerWriteBasic(Datas.STRING, Datas.STRING,Datas.STRING)
                    , SerializerReadDouble.INSTANCE);

            d = (Double) p.find(productID, unitId, unitToId);
    ...
    public final UnitInfo getUnitInfo(String id) throws BasicException {
        ...
        /**
     * Reurn the location string of 1 product id
     * @param id
     * @return UnitInfo
     * @throws BasicException 
     */
    public final String getLocationProduct(String id) throws BasicException {
    ...
    /**
     * regresa la descripcion de la unidad por defecto para el producto - retuns default unit by product id
     * @param productId
     * @return String
     * @throws BasicException 
     */
    public final String getDefaultUnitTextProductID(final String productId) throws BasicException {
        return (String) new PreparedSentence(s
            , "SELECT u.name  " +
              "  FROM additional_prices_for_products ap" +
              "  JOIN units u ON (ap.unit_to_id  = u.id  )  where ap.product_id = ?" // and ap.is_default = true"
            , SerializerWriteString.INSTANCE
            , new SerializerRead() { public Object readValues(DataRead dr) throws BasicException {
                return dr.getString(1);
            }}).find(productId);
     ...
     * @throws BasicException 
     */
    public final InsuranceInfo getInsuranceInfo(String customerId) throws BasicException {
        return (InsuranceInfo) new PreparedSentence(s
            , " SELECT i.id,p.copay_value,p.copay_percentage,p.name,i.smj_insurancecompany_id,i.name    from customers c "+
              " JOIN smj_insurancecompany i ON (i.id  = c.smj_insurancecompany_id   ) "+
              " JOIN smj_insuranceplan p ON (p.smj_insuranceplan_id  = c.smj_insuranceplan_id  ) "+
              " WHERE c.id = ? "              
            , SerializerWriteString.INSTANCE
            , InsuranceInfo.getSerializerRead()).find(customerId);
    }
    
        // Listados para combo
    public final TaxInfo getTaxInfoByName(String name)   throws BasicException{
        return (TaxInfo)new  PreparedSentence(s
            , "SELECT ID, NAME, CATEGORY, VALIDFROM, CUSTCATEGORY, PARENTID, RATE, RATECASCADE, RATEORDER FROM TAXES " +
              " WHERE NAME = ? "              
            , SerializerWriteString.INSTANCE
            , TaxInfo.getSerializerRead()).find(name);
    }
    
    public final TicketProductNote getTicketProductNote(String ticketId, String productId) throws BasicException {
        //String[] data={ticketId,productId};
        Object obj= new PreparedSentence(s
            , "SELECT * FROM TICKET_PRODUCT_NOTE t WHERE t.TICKET_ID=? AND t.PRODUCT_ID=?"
            , SerializerWriteString.INSTANCE
            , TicketProductNote.getSerializerRead()).find(ticketId,productId);
        System.out.println(" object "+obj);
        return (TicketProductNote)obj;
    }
