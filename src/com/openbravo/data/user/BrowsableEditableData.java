...
//        m_keyvalue = DocumentLoaderBasic.INSTANCE;
        ...
        if (m_Dirty.isDirty()) {
            if (m_iState == ST_UPDATE) {
                if(canSave()){
                    ...
                    // SmartPOS
                    // update the current customer, so the sales window can reuse it
                     if(m_editorrecord instanceof  CustomersView)
                     {
                        CustomersView cv1 = ((CustomersView)m_editorrecord);
                        JRootApp.currentCustomer = cv1.getM_jTaxID().getText();
                     }
                    m_editorrecord.refresh();
                    baseMoveTo(i);
                }else{
                ...
                if(canSave()){
                    ...
                    // SmartPOS
                     ...
                     {
                        CustomersView cv1 = ((CustomersView)m_editorrecord);
                        //System.out.println("Salvada informacion del cliente:"+cv1.getM_jTaxID().getText());                   
                        JRootApp.currentCustomer = cv1.getM_jTaxID().getText();
                     }
    ...
    /**
     * Send customer info to the ERP - SmartPOS
     */
